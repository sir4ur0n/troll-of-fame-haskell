module TrollSpec where

import qualified Troll as SUT
import Elf
...
test_inverse = testProperty "A troll killing an elf and then realizing the elf actually survived remains unchanged" \troll elf ->
  let
    trollWithAdditionalElfKilled = SUT.iGotOne elf troll
...
test_analogy = testProperty "Killing N elves one by one is the same as killing N elves in a single strike" \troll elf (Positive nbElves) ->
  let
    oneByOne = replicate (fromInteger $ SUT.getInteger nbElves) elf
...
test_idempotent = testProperty "Resurrected elves cannot be resurrected again" \ troll elf ->
  let 
    resurrectedOnce = SUT.allElvesOfAKindResurrected elf troll
    resurrectedTwice = SUT.allElvesOfAKindResurrected elf resurrectedOnce
  in 
    resurrectedOnce === resurrectedTwice
...
test_metamorphic = testProperty "When a troll kills an elf, his score should increase" \troll elf ->
  let
    scoreBefore = SUT.score troll
...
test_injective = testProperty "When killing elves with different roles, a troll should get different scores" \troll elf1 elf2 ->
  elf1 /= elf2 ==> 
    ...
    in SUT.score trollKilledElf1 =/= SUT.score trollKilledElf2